...
{
    lastsock= ((real_socket_t) dlsym(RTLD_NEXT, "socket")) (domain, type, protocol);
    return  lastsock;
}
...
ssize_t real_read(int sockfd, void *buf, size_t len)
{
    return ((real_read_t) dlsym(RTLD_NEXT, "read")) (sockfd, buf, len);
}
...
ssize_t read(int sockfd, void *buf, size_t len)
{
    ...
    if (sockfd>2) {
            printf("[-] Read using `read` from socket (fd: %d)\n", sockfd);
                fwrite(buf, sizeof(char), BUF_PREVIEW_LEN, stdout);
                puts("[...]\n");
    }
...
ssize_t real_write(int sockfd, const void *buf, size_t len)
{
    return ((real_write_t) dlsym(RTLD_NEXT, "write")) (sockfd, buf, len);
}
...
ssize_t write(int sockfd, const void *buf, size_t len)
{
    ...
    if (sockfd>2) {
                printf("[-] Write using `write` from socket (fd: %d)\n", sockfd);
                fwrite(buf, sizeof(char), ret, stdout);
        }
